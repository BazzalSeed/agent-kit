...
3. **Spawn ALL teammates at once** — one Agent-tool call per roles-table row, in a single message, each a distinct `name`. A team is strongest with everyone present from the start; don't stagger-spawn sequential roles — handle dependencies with **prep-then-wait** instructions instead.
   ...
   - **Use the bundled role agents when a plan role matches one.** Spawn via `subagent_type` — `agent-team:foundation-guardian`, `agent-team:builder`, `agent-team:reviewer` — and each carries its standing discipline automatically (the foundation-guardian's freeze-then-guard + no-feature-behavior + verify-before-rebuild; the builder's TDD + "done" = tests green; the reviewer's milestone cadence, hands-on end-to-end final gate, and the two false-pass traps). You then inject only the **per-run specifics** in the spawn prompt: the Ship goal, the owned area, seam-partner names, any gate / prep-then-wait signal, and plan-approval criteria. (If a bundled agent isn't resolvable this session, inline the role's brief instead.) Two per-run reminders the agents can't know on their own: tell the **foundation-guardian** the builders' names so they route seam changes to it, and confirm the plan handed the **reviewer** a drivable test seam to reach gated/authed flows.
