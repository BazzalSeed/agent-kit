...
3. **Spawn ALL teammates at once** — one Agent-tool call per roles-table row, in a single message, each a distinct `name`. A team is strongest with everyone present from the start; don't stagger-spawn sequential roles — handle dependencies with **prep-then-wait** instructions instead.
   ...
   - **Foundation/gate roles: freeze contracts + skeleton + deploy/infra wiring, and verify-before-rebuild.** Bake in: "stand up only the shared skeleton/scaffold/schema/config **plus the deploy/infra wiring (project + env + domains + hosting)** needed to unblock the lanes + freeze the contracts as stubs; shared provisioning that runs once before the lanes fork is legitimately yours, but do NOT implement feature behavior." And for resumed runs: "if foundation already exists in the repo, **inspect and verify/finish it — do not blindly re-run setup** (re-scaffolding into an existing app errors)."
     - **When the foundation is already built, re-scope this role from *builder* to *guardian* — don't drop it.** Bake in: "the foundation exists; **verify it's green (verify-before-rebuild), then act as the standing sole owner of changes to the frozen seam** — contracts/interfaces, schema/migrations, infra/env. Builders route every contract/schema/infra change through you; you review & apply them and wire user-supplied secrets into env. Still implement **no feature behavior**." Inject the builders' names so they know to message it for changes.
   - **Builders:** TDD + run their own unit/integration tests green before reporting "done" (bake into the prompt).
   - **Reviewer:** review at the plan's **milestones** (not per-task) — typically the foundation/contracts gate, the integration seam, and a final birds-eye. **Bake into the prompt that the FINAL gate is hands-on end-to-end testing of every in-scope feature/workflow** — actually drive the running product (browser via the cmux browser / CLI / API), not just code-read + unit tests — and that the gate report must list **"exercised e2e" vs. "couldn't verify (with reason)"**, never claiming a flow it didn't run. Flows that can't be driven headless (real audio, interactive OAuth, live webhooks) get **flagged as unverified, not rubber-stamped**; make sure the plan gives the reviewer a **drivable test seam** (e.g. a guarded dev-login) to reach authed/gated flows. Also bake in the two false-pass traps: **(a) DOM/exit-code present ≠ correct** — screenshot-eyeball visual/output claims (a placeholder or broken image still has a present `<img>`+alt), don't pass on element-presence alone; **(b) seed/demo/fixture data ≠ the real path** — re-drive against real data (real DB row / upload / API call) before marking a flow verified.
